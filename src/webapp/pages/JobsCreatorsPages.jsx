...
import { ArrowLeft, BriefcaseBusiness, Building2, CalendarDays, ChevronRight, FileText, MapPin, Megaphone, Search, UsersRound, WalletCards } from 'lucide-react'
...
export function JobDetailPage() {
  ...
  async function apply(event) {
    ...
    setApplying(true)
    try {
      await api.applyForJob(jobId, { coverLetter: form.get('coverLetter'), ...(form.get('resumeUrl') ? { resumeUrl: form.get('resumeUrl') } : {}) })
...
export function CreatorsPage() {
  ...
  return <div className="app-page"><PageIntro eyebrow="UGC creators" title="Find creators who understand your audience" description="Discover available creators by niche, audience, and portfolio." actions={<Link className="app-button app-button--primary" to="/app/creators/become">Become a creator</Link>} /><label className="creator-search"><Search size={19} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search niche, bio, or creator" /></label>{state.loading ? <LoadingState label="Loading creators" /> : state.error ? <ErrorState message={state.error} retry={state.reload} /> : <><div className="niche-chips">{niches.map((item) => <button type="button" className={item === niche ? 'is-active' : ''} key={item} onClick={() => setNiche(item)}>{item}</button>)}</div>{filtered.length ? <div className="creator-grid">{filtered.map((creator) => { const name = creatorName(creator); return <Link key={creator.id} to={`/app/creators/${creator.id}`} className="creator-card"><div className="creator-card__top"><Avatar name={name} src={creator.user?.avatarUrl} size="large" /><span className={`availability ${creator.isAvailable ? 'is-available' : ''}`}>{creator.isAvailable ? 'Available' : 'Unavailable'}</span></div><h2>{name}</h2><p>{creator.bio || 'UGC creator on Zidash'}</p><div className="niche-row">{creatorNiches(creator).slice(0, 3).map((item) => <span key={item}>{item}</span>)}</div><div className="creator-stats"><span><strong>{Number(creator.followersCount || 0).toLocaleString()}</strong> followers</span><span><strong>{Number(creator.engagementRate || 0)}%</strong> engagement</span></div></Link> })}</div> : <EmptyState icon={UsersRound} title="No creators match your search" message="Try another niche or clear the search." />}</>}</div>
}
