...
export function MarketplaceHome() {
  ...
  return <div className="app-page marketplace-home">
    ...
    <section className="app-section">
      ...
      {taxonomy.loading ? <div className="category-skeletons dashboard-category-skeletons">{Array.from({ length: 8 }, (_, index) => <span key={index} />)}</div> : taxonomy.error ? <ErrorState message="Categories could not be loaded." retry={taxonomy.reload} /> : categories.length ? <div className="app-category-row dashboard-category-row">{categories.slice(0, 7).map((category) => <Link className="app-category-tile" key={category.id} to={`/app/category/${category.id}`}><span><CategoryMark category={category} /></span><strong>{category.name}</strong><small>{category.children?.length || 0} subcategories</small></Link>)}<Link className="app-category-see-all" to="/app/categories"><span><img src="/categories-all-3d.png" alt="" /></span><strong>See all categories</strong><small>Browse everything</small></Link></div> : <EmptyState icon={Boxes} title="Categories could not be loaded" message="No active marketplace categories were returned." action={<button type="button" className="app-button app-button--outline" onClick={taxonomy.reload}>Retry</button>} />}
    </section>

    <section className="app-shortcuts" aria-label="More on Zidash">
      <Link to="/app/jobs"><span className="app-shortcuts__icon"><img src="/shortcut-jobs-3d.png" alt="" /></span><div><strong>Jobs & talent</strong><span>Find work or hire locally</span></div><ChevronRight /></Link>
      <Link to="/app/creators"><span className="app-shortcuts__icon"><img src="/shortcut-ugc-3d.png" alt="" /></span><div><strong>UGC creators</strong><span>Find creators for your brand</span></div><ChevronRight /></Link>
      <Link to="/app/community"><span className="app-shortcuts__icon"><img src="/shortcut-community-3d.png" alt="" /></span><div><strong>Communities</strong><span>Discover posts across Nigeria</span></div><ChevronRight /></Link>
    </section>
    ...
    <aside className="seller-callout"><div><span className="seller-callout__icon"><img src="/sell-opportunity-3d.png" alt="" /></span><div><strong>Turn unused items into opportunity.</strong><p>Create a listing, reach local buyers, and boost it when you want more visibility.</p></div></div><Link className="app-button app-button--primary" to="/app/sell">Start selling</Link></aside>
    ...
    <aside className="promote-post-callout">
      <div className="promote-post-callout__icon"><img src="/promote-post-3d.png" alt="" /></div>
...
export function SellerPage() {
  ...
  return <div className="app-page"><section className="store-hero" style={profile.coverImageUrl ? { backgroundImage: `linear-gradient(90deg, rgba(15,35,17,.86), rgba(15,35,17,.35)), url(${profile.coverImageUrl})` } : undefined}><div className="store-avatar">{profile.displayName?.slice(0, 2).toUpperCase()}</div><div><span className="app-eyebrow">Zidash store</span><h1>{profile.displayName}</h1><p>{profile.bio || 'Local seller on Zidash'}</p><div className="store-meta"><span><MapPin size={15} /> {profile.location || 'Nigeria'}</span><span><BadgeCheck size={15} /> Trust score {profile.trustScore || 0}%</span><span><Star size={15} /> {profile.ratingAverage || 0} ({profile.ratingCount || 0})</span></div></div><div className="store-hero__actions">{!isOwnStore && <button type="button" className={`app-button ${following ? 'app-button--outline-light' : 'app-button--light'}`} onClick={toggleFollow}>{following ? 'Following' : 'Follow seller'}</button>}{!isOwnStore && <button type="button" className="store-report-button" onClick={openSellerReport}><CircleAlert size={16} /> Report seller</button>}</div></section><section className="app-section"><SectionHeading title="Listings from this seller" subtitle={`Member since ${shortDate(profile.memberSince)}`} />{listings.loading ? <LoadingState /> : listings.error ? <ErrorState message={listings.error} retry={listings.reload} /> : <ListingGrid listings={unwrapItems(listings)} emptyTitle="This seller has no active listings" />}</section>{reportOpen && <ReportDialog title="Report seller" subjectLabel="seller" submitting={reporting} onClose={() => setReportOpen(false)} onSubmit={reportSeller} />}</div>
}
